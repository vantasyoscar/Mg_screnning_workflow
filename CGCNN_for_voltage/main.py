...
#not args.disable_cuda and torch.cuda.is_available()
...
def train(train_loader, model, criterion, optimizer, epoch, normalizer):
    ...
    for i, (input, target, batch_cif_ids, atom_table_num) in enumerate(train_loader):
        ...
        if args.cuda:
            input_var = (Variable(input[0].cuda(non_blocking=True)),
                        ...
                        atom_table_num.cuda(non_blocking=True))
        else:
...
def validate(val_loader, model, criterion, normalizer, test=False):
    ...
    for i, (input, target, batch_cif_ids, atom_table_num) in enumerate(val_loader):
        if args.cuda:
            with torch.no_grad():
                input_var = (Variable(input[0].cuda(non_blocking=True)),
                             Variable(input[1].cuda(non_blocking=True)),
